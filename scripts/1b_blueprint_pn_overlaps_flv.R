...
flv_reservoirs <- c("Apodemus flavicollis",
...
pseudo_reservoir_pool <- c(unique(virion_m_clean %>% 
                                    filter(VirusGenus %in% c("flavivirus") & Virus %!in% c("bagaza virus", "gadgets gully virus", "ilheus virus", "israel turkey meningoencephalomyelitis virus", "japanese encephalitis virus", "meaban virus", "murray valley encephalitis virus", "saint louis encephalitis virus", "tembusu virus", "tyuleniy virus", "usutu virus", "west nile virus", "cacipacore virus") & Host %!in% flv_reservoirs) %>%
                                    pull(Host)))

positive_species <- c(pseudo_reservoir_pool, flv_reservoirs)
...
# tab_overlaps <- NULL
tab_overlaps <- read.csv(".../data/tab_overlaps_flv.csv")
...
endem_map <- ecoregions_transformed_reproj
