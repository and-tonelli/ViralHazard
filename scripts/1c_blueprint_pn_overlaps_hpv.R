...
pseudo_reservoir_pool <- c(unique(virion_m_clean %>% filter(VirusGenus == "henipavirus" & Host %!in% hpv_reservoirs) %>%
                                    pull(Host)), "Apodemus agrarius", "Myodes rutilus", "Crocidura lasiura", "Crocidura shantungensis", "Equus ferus", "Sus scrofa")

# Adding Langya Virus: https://www.nejm.org/doi/suppl/10.1056/NEJMc2202705/suppl_file/nejmc2202705_appendix.pdf and S. scrofa and E. caballus
positive_species <- c(pseudo_reservoir_pool, hpv_reservoirs)
...
endem_map <- ecoregions_transformed_reproj[ecoregions_transformed_reproj$Regions %in% c("African", "Guineo-Congolian", "Indo-Malayan", "Oriental", "Chinese", "Australian"), ]
