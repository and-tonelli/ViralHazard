...
dataset <- read.csv(".../data/phv_dataset_main.csv") 

# cor(dataset[c(5:11, 13:31)]) %>% 
#   as.data.frame(.) %>%
...
dataset <- dataset[-c(9, 6, 11, 20, 27, 28, 7, 31, 10, 14, 22, 17, 25, 29, 21, 24, 26, 6, 11, 19, 27)] 
...
task.pn = as_task_classif(id = "pn", reformulate(names(dataset[-c(1, 2, 4, 14)]), names(dataset[14])), data = dataset, positive = "1")
...
learner.gam = lrn("classif.gam",
                  formula = status ~ order.Carnivora + order.Cetartiodactyla + order.Chiroptera + order.Didelphimorphia + order.Rodentia + order.Eulipotyphla + order.Perissodactyla + order.Pilosa + order.Proboscidea+
                    trophic_level.3 + trophic_level.2 + s(adult_mass_g) + 
                    s(litter_size_n) + s(mean_divergence) + s(area.1) + 
                    s(mean.wc2.1_5m_bio_9) + s(mean.wc2.1_5m_bio_16)+
                    s(stdev.wc2.1_5m_bio_8) + s(mean.wc2.1_5m_bio_8),
                  select = TRUE,
                  method = "REML",
                  gamma = to_tune(p_dbl(1, 1)),
                  predict_type = "prob")
...
saveRDS(bench_stack_results, file = ".../data/phv_stack_model.rds")
# bench_stack_results <- readRDS(file = "phv_stack_mod.rds")
...
phylo <- read_csv(".../data/phvmean_phylo_dist_to_known_hosts.csv")
...
write.csv(predictions, ".../data/phv_predictions_outofsample.csv", row.names = F)

#### Predictions for in sample species (high- and low- evidence hosts, pseudo-negatives) ####
dataset <- read.csv(".../data/phv_dataset_main.csv")
bench_stack_results <- readRDS("phv_stack_model.rds")
...
predictions_outofsample <- read.csv(".../data/phv_predictions_outofsample.csv")
...
write.csv(predictions_both, ".../data/phv_predictions_main.csv", row.names = F)
