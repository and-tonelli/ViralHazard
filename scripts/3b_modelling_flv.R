...
dataset <- read.csv(".../data/flv_dataset_main.csv") 

cor(dataset[-c(1:4, 11, 21, 20)])
...
dataset <- dataset[-c(30, 10, 14, 22, 17, 25, 15, 23, 7, 18, 26, 19, 27)]
...
task.pn = as_task_classif(id = "pn", reformulate(names(dataset[-c(1, 2, 4, 19)]), names(dataset[19])), data = dataset, positive = "1")
...
learner.gam = lrn("classif.gam",
                  formula = status ~ order.Carnivora + order.Cetartiodactyla + order.Chiroptera + order.Didelphimorphia + order.Rodentia + order.Eulipotyphla + order.Primates + order.Pilosa + order.Diprotodontia+ order.Lagomorpha +
                    trophic_level.3 + trophic_level.2 + s(adult_mass_g) + s(max_longevity_d) +
                    s(litter_size_n) + s(litters_per_year_n) + s(weaning_age_d) + s(mean_dist) + s(area.1) + 
                    s(mean.wc2.1_5m_bio_9) + s(mean.wc2.1_5m_bio_18) +
                    s(stdev.wc2.1_5m_bio_18) + s(stdev.wc2.1_5m_bio_9) +
                    s(mean.wc2.1_5m_bio_19) + s(stdev.wc2.1_5m_bio_19),
                  select = TRUE,
                  method = "REML",
...
saveRDS(bench_stack_results, file = ".../data/flv_stack_model.rds")
# bench_stack_results <- readRDS(file = "flv_stack_mod.rds")
...
phylo <- read_csv(".../data/flvmean_phylo_dist_to_known_hosts.csv")
...
write.csv(predictions, ".../data/flv_predictions_outofsample.csv", row.names = F)

#### Predictions for in sample species (high- and low- evidence hosts, pseudo-negatives) ####
dataset <- read.csv(".../data/flv_dataset_main.csv")
bench_stack_results <- readRDS(".../data/flv_stack_model.rds")
...
predictions_outofsample <- read.csv(".../data/flv_predictions_outofsample.csv")
...
write.csv(predictions_both, ".../data/flv_predictions_main.csv", row.names = F)
