...
phv_reservoirs <- c("Arvicanthis niloticus",
...
combine$iucn2020_binomial[combine$iucn2020_binomial == "Micaelamys namaquensis"] <- "Aethomys namaquensis"
...
positive_species <- c(pseudo_reservoir_pool, phv_reservoirs)
...
# tab_overlaps <- NULL
tab_overlaps <- read.csv(".../data/tab_overlaps_phv.csv")
...
endem_map <- ecoregions_transformed_reproj[ecoregions_transformed_reproj$Regions %in% c("African", "Guineo-Congolian", "Panamanian", "Amazonian", "North American", "South American", "Eurasian", "Oriental"), ]
