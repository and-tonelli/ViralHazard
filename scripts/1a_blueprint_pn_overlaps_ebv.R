...
combine <- read.csv(".../datacombine_imputed.csv") %>% filter(iucn2020_binomial != "Not recognised")
...
positive_species <- c(pseudo_reservoir_pool, ebv_reservoirs)
...
# (if host status is unknown put 0 when the species' range overlaps (50% threshold) with that of one or more positive species belonging to the same family)
...
endem_map <- ecoregions_transformed_reproj[ecoregions_transformed_reproj$Regions %in% c("African", "Guineo-Congolian", "Indo-Malayan", "Oriental", "Chinese"), ]
