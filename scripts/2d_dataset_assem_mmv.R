...
mmv_reservoirs <- c("Calomys musculinus", "Mastomys natalensis", "Mus musculus", 
                    "Rattus norvegicus", "Peromyscus californicus", "Calomys callosus")

# Checking if all of them are in COMBINE
combine <- read.csv(".../data/combine_imputed.csv")

unique(combine %>% 
         filter(phylacine_binomial %in% mmv_reservoirs) %>% 
...
# How many species are associated to mmv just via Serology?
virion_m_clean %>% 
  filter(VirusGenus %in% c("mammarenavirus") & Host %!in% ebv_reservoirs, DetectionMethod == "Antibodies") %>% 
...
associated_just_via_antibodies <- setdiff(antibodies, species) # 2 species: "Chionomys roberti" "Rattus norvegicus"

# Getting low-evidence hosts (i.e., potentially unrecognized reservoirs)
# These are the species that tested positive but are not high-evidence (according to WOS search)
...
pseudo_reservoir_pool <- c(unique(virion_m_clean %>% filter(VirusGenus %in% c("mammarenavirus") & Host %!in% mmv_reservoirs) %>%
                                    pull(Host)))
...
dist_known_hosts <- read_csv(".../data/mmvmean_phylo_dist_to_known_hosts.csv")
...
highevidence_set <- phyclimbine %>% filter(phylacine_binomial %in% mmv_reservoirs) %>% 
  merge.data.frame(., dist_known_hosts, by.x = 1, by.y = 1) %>% 
  mutate(status = 1) %>% 
  dplyr::select(c(phylacine_binomial, iucn2020_binomial, order, family, 
                  adult_mass_g, max_longevity_d, gestation_length_d, litter_size_n, litters_per_year_n, 
...
species_level_re <- virion_m_clean %>% 
  filter(VirusGenus %in% c("mammarenavirus")) %>% 
  group_by(Host) %>%
  summarise(records = n())
...
mmv_lowevidence_set <- lowevidence_set %>%
...
mmv_dataset <- rbind(highevidence_set_w[-35] %>% relocate(w, .after = last_col()),
                     mmv_lowevidence_set,
...
sum(mmv_dataset$w[mmv_dataset$status == "0"])
sum(mmv_dataset$w[mmv_dataset$status == "1"])

write.csv(mmv_dataset, ".../data/mmv_dataset_main.csv", row.names = F)
