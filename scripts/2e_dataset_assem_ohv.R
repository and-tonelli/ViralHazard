...
unique(combine %>% 
         filter(phylacine_binomial %in% ohv_reservoirs) %>% 
...
# How many species are associated to ohv just via Serology?
virion_m_clean %>% 
  filter(VirusGenus %in% c("orthonairovirus") & Host %!in% ohv_reservoirs, DetectionMethod == "Antibodies") %>% 
...
pseudo_reservoir_pool <- c(unique(virion_m_clean %>% 
                                    filter(VirusGenus %in% c("orthonairovirus") & Virus %!in% c("hughes orthonairovirus", "sakhalin orthonairovirus") & Host %!in% ohv_reservoirs) %>%
                                    pull(Host)))
...
dist_known_hosts <- read_csv(".../data/ohvmean_phylo_dist_to_known_hosts.csv")
...
species_level_re <- virion_m_clean %>% 
  filter(VirusGenus %in% c("orthonairovirus")) %>% 
  group_by(Host) %>%
  summarise(records = n())
...
ohv_lowevidence_set <- lowevidence_set %>%
...
ohv_dataset <- rbind(ohv_lowevidence_set,
...
sum(ohv_dataset$w[ohv_dataset$status == "0"])
sum(ohv_dataset$w[ohv_dataset$status == "1"])

write.csv(ohv_dataset, ".../data/ohv_dataset_main.csv", row.names = F)