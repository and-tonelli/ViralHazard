# Filoviruses (Ebolavirus + Marburgvirus)

require(tidyverse)
require(mlr3)

dataset <- read.csv(".../data/ebv_dataset_main.csv") 

# Variable preparation
names(dataset)

# Remove variables with a pairwise |correlation| >= 0.7
dataset <- dataset[-c(10, 6, 7, 9, 21, 11, 14, 22, 15, 23, 16, 24, 26, 27, 31)]
...
learner.gam = lrn("classif.gam",
                  formula = status ~ order.Cetartiodactyla + order.Chiroptera + order.Rodentia + order.Eulipotyphla + order.Primates+
                    trophic_level.3 + trophic_level.2 + s(adult_mass_g) +
                    s(litter_size_n) + s(area.1) + s(mean_divergence) +
                    s(mean.wc2.1_5m_bio_10) + s(mean.wc2.1_5m_bio_18)+ s(mean.wc2.1_5m_bio_16) + s(mean.wc2.1_5m_bio_17)+ 
                    s(stdev.wc2.1_5m_bio_10) + s(stdev.wc2.1_5m_bio_18) + s(stdev.wc2.1_5m_bio_19),
                  select = TRUE,
                  method = "REML",
...
saveRDS(bench_stack_results, file = ".../data/ebv_stack_model.rds")
...
phylo <- read_csv(".../data/ebvmean_phylo_dist_to_known_hosts.csv")
...
write.csv(predictions, ".../data/ebv_predictions_outofsample.csv", row.names = F)

#### Predictions for in sample species (high- and low- evidence hosts, pseudo-negatives) ####
dataset <- read.csv(".../data/ebv_dataset_main.csv")
bench_stack_results <- readRDS(".../data/ebv_stack_model.rds")
...
predictions_outofsample <- read.csv(".../data/ebv_predictions_outofsample.csv")
...
write.csv(predictions_both, ".../data/ebv_predictions_main.csv", row.names = F)
